...
// One full earn -> settle -> rate pass over a single deal, driven by hand. Returns the
// cut that reached the fund, or null if the deal could not be processed.
//
// This is no longer how the fund earns. Revenue used to be computed from a startup's
// quality and paid by a single house customer, with the deal's own judge rating the
// result. Both halves of that are gone: market.ts has independent customer agents decide
// what to buy, and the buyers do the rating. Kept as an operator escape hatch for forcing
// a single deal to settle when something is stuck.
